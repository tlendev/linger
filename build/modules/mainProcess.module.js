...
const mainProcess = async (page)=>{
    ...
    const startingProgress = +await page.$eval('body > div.container-top > div.text-center > div > h5', (node)=>{
    ...
    while(progress < 20){
        ...
        progress = +await page.$eval('body > div.container-top > div.text-center > div > h5', (node)=>{
            return node.innerHTML.trim().match(/(.+)\/20/)[1]; // @returns XX/20
        });
    }
};
